...
def main(input_filepath, seq_len, prediction_seq_len, batch_size, epochs):
    x_note, x_len, y_note, y_len = getData(
        input_filepath, seq_len, prediction_seq_len)
    ...
    model = train(model, x_note_train, x_len_train,
                  y_note_train, y_len_train, batch_size, epochs)

    # plot_model(model, to_file='model.png')

    # Score model
    score = model.evaluate([x_note_test, x_len_test], [
                           y_note_test, y_len_test], verbose=1)
    i = 0
...
def train(model, x_note_train, x_len_train, y_note_train, y_len_train, batch_size, epochs):
    ...
    tensorboard = TensorBoard(log_dir="./logs/tensorboard", histogram_freq=0, write_graph=True, write_grads=True,
                              write_images=True, embeddings_freq=0, embeddings_layer_names=None, embeddings_metadata=None, embeddings_data=None)
    earlystopping = EarlyStopping(monitor='acc', min_delta=0, patience=1,
                                  verbose=0, mode='auto', baseline=None, restore_best_weights=False)

    # unique file name that will include the epoch and the validation acc for that epoch
    filepath = "../../model-checkpoint.h5"
...
def getModel():
    ...
    output_length = Dense(53, activation="softmax",
                          name="output_length")(model)

    model = Model(inputs=[input_notes, input_length],
                  outputs=[output_note, output_length])
...
def createXY(arr, sequenceLength, predictionLength):
    ...
    while i + sequenceLength + predictionLength < len(arr):
        ...
        y.append(arr[i + sequenceLength: i +
                     sequenceLength + predictionLength])
        i += sequenceLength
